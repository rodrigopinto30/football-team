  ...
  return (
    <div className='flex flex-col gap-6 w-[60%] mt-[5%]'>
      ...
      <div className='grid grid-cols-4 gap-6'>
        {teams.length == 0 ? 
          <div>
            <h4 className='font-mont font-medium text-white text-center text-md'>There are not teams</h4>
          </div> 
        : 
          teams.map((team)=>(
            <div 
              ...
              {editingTeamId === team.id ? (
                <div className="flex flex-col gap-2">
                  <div className='w-full flex flex-col'>
                    <span className='font-mont font-thin text-gray-100'>Team</span>
                    <input 
                    ...
                    <fieldset className='flex flex-col'>
                      <legend className='font-mont font-thin text-gray-100'>Does it havea stadium?</legend>
                      <label 
                        htmlFor="stadium-yes"
                    ...
                    <fieldset>
                      <span className='font-mont font-thin text-gray-100'>DT</span>
                      <span className='font-mont font-medium text-white'>{team.dt?.name}</span>
                    </fieldset>

                    <fieldset>
                      <span className='font-mont font-thin text-gray-100'>Players</span>
                      <div>
                ...
                <div className='flex flex-col gap-2'>
                  <div className='w-full flex flex-col'>
                    <span className='font-mont font-thin text-gray-100'>Team</span>
                    <span className='font-mont font-medium text-white'>{team.name}</span>
                  </div>
                  <div className='w-full flex flex-col'>
                        ...
                        {team.players?.map((juagador)=>(
                          <li>
                            <span className='font-mont text-gray-400'>
                              Name: 
                                <span className='text-white'>{juagador.numbers}</span>
                              </span>
                            <span className='font-mont text-gray-400'>
                              Position: 
                                <span className='text-white'>{juagador.position}</span>
                              </span>
                          </li>
                        ))}
                      </ul>
